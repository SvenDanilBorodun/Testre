...
- **Aufnahme recording RAM is capped at 4 GB by the safety valve, not by architecture.** The recording path decodes each `CompressedImage` to a 640×480 RGB uint8 ndarray (~921 KB/frame) and appends to `lerobot_dataset_wrapper.episode_buffer` until `save_episode_*` runs end-of-episode. At 30 Hz × 2 cameras that's ~1.8 MB/tick → unbounded growth → OOMkill at ~110 s on the `mem_limit: 6g`. The 2026-05 safety valve (`_episode_image_bytes` counter + `_buffer_full` flag in `lerobot_dataset_wrapper.py`, polled in `data_manager.py::run` state) forces a save when the buffer crosses `EDUBOTICS_MAX_BUFFER_GB` (default 4 GB). Hard upper bound on episode length: **~110 s of 30 Hz dual-camera recording at 640×480** before the valve fires. The proper fix (store JPEG bytes during recording, decode chunked inside `FFmpegEncoder`) is deferred to v2.4 — ~50× headroom. If you raise the buffer cap, also raise `mem_limit` in `docker-compose.yml` to leave headroom for the rest of the node.
