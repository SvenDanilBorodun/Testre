...
**ONE sanctioned exception (v2.6.0, user-approved): the teleop force/collision e-stop.** Scoped to teleop/recording ONLY — `CollisionDetector.update()` is hard-gated on `mode_is_inference` AND the only `/collision_flag` consumer (the leader broadcaster) is not in the inference action path, so the guard is doubly invisible to inference and never reshapes the recorded→replayed action distribution. It never modifies samples either: a trip DISCARDS the in-progress episode (`re_record()`), it does not filter or clamp. Full mechanics under "Critical architectural choices".
