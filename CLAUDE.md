...
>
> Last verified by reading every load-bearing file directly: **2026-05-17** (v2.3.0 follow-up audit fixes — 5 parallel Opus 4.7 1M-context deep-dive agents reviewed every commit from `62f8e4f` to `0362952` against live Supabase / Railway / Docker Hub state. Findings landed in this commit: **B1 BLOCKER** `rosbridge_proxy.py:101` JWKS URL was `/auth/v1/keys` which returns 401 on modern Supabase — every ES256/RS256 student WS auth was silently closing with 4401 → fixed to `/auth/v1/.well-known/jwks.json` (curl-verified returns 200 with the project's EC P-256 public key) · **B2 BLOCKER** `robotis_ai_setup/docker/.env.build:5` still pointed `CLOUD_API_URL` at the dead `production-9efd` slug (commit `caf6ab2` missed this gitignored defaults file) → next local `build-images.sh` would have re-baked the dead URL into a student bundle, all Cloud-API calls 404 · **B3 BLOCKER** `physical_ai_manager/src/pages/TrainingPage.js:112` read `window.__edubotics_jetson_id__` which is never set anywhere in `src/` — the 25e54d2 "TrainingPage logout fix" was therefore dead code; now reads `state.jetson.jetsonId` from Redux exactly like StudentApp.js · **N1 hardening** `rosbridge_proxy._verify_jwt` now pins `issuer=SUPABASE_URL/auth/v1` on both the ES256/RS256 JWKS path and the HS256 shared-secret path, closing the (vanishingly unlikely) "leaked token from another Supabase project sharing a JWKS kid by chance" attack · **CLAUDE.md drift swept**: §3 corrected from "9 layers" to "10 layers" with a new Classroom Jetson row, §4 inventory now lists the 4 new `nettername/*-jetson*` repos + bumps Supabase table count 10→11 (adds `jetsons`), §9 header corrected from "14 numbered migrations" to "16 active migrations through 020" + new §9.19a documents migration 020, §14 stale "arm64-latest" wording removed and the version-bump rule extended to require the Railway `GUI_VERSION` + `GUI_DOWNLOAD_URL` env vars AND a matching GitHub release `v<version>` with the `.exe` asset (without which the in-tree VERSION bump is invisible to existing installs) · **Three follow-ups remain user-side**: (a) build EduBotics_Setup.exe v2.3.0 on Windows and `gh release create v2.3.0 --title "EduBotics 2.3.0" ./installer/output/EduBotics_Setup.exe`, then (b) bump Railway env `GUI_VERSION=2.3.0` and `GUI_DOWNLOAD_URL=https://github.com/SvenDanilBorodun/Testre/releases/download/v2.3.0/EduBotics_Setup.exe` on `scintillating-empathy`, (c) rebuild + push `nettername/physical-ai-manager:latest` so the TrainingPage Redux-jetsonId fix actually reaches student installs). Earlier 2026-05-17: v2.3.0 follow-up: migration 020 with `agent_release_jetson` + `regenerate_pairing_code` + `unpair_jetson` RPCs · Cloud API gains `POST /jetson/{id}/agent-release` + `POST /jetson/{id}/release-beacon` + `POST /teacher/classrooms/{id}/jetson/regenerate-code` + `POST /teacher/classrooms/{id}/jetson/unpair` · `/jetson/register` forwards `SUPABASE_JWT_SECRET` to the agent when HS256 + boot warning when configured wrong · `/jetson/` rate limit bumped 10→30/60s to absorb classroom-of-30-students tab-close storms on the beacon path · schema fingerprint extended to probe all 3 new RPCs · teacher dashboard gains `JetsonSection` card + `PairJetsonModal` + `useTeacherJetsonRealtime` subscription so the teacher can pair / force-release / regenerate-code / unpair from the classroom detail page · React `releaseJetsonBeacon` rewritten to hit the new dedicated endpoint with an `application/json` Blob body (was silently failing auth before) · `resetJetsonOnLogout` wired into all 5 signOut callsites in `StudentApp.js` + `WebApp.js` so logout releases the lock instead of leaking it · transient heartbeat-failure UI variant ("Verbindung wird wiederhergestellt…") after 2 consecutive non-410 failures · setup.sh reads `supabase_jwt_secret` from register response + defensive abort if HS256-but-no-secret · agent.py auto-pull rewritten to per-attempt 180 s timeout × 3 retries with [0/5/15 s] backoff (was monolithic 600 s) · Jetson Docker repos renamed per Sven's directive: `nettername/{open-manipulator,physical-ai-server}-jetson{,-base}` instead of `arm64-` tag suffixes on the existing repos · 9 new cloud_training_api tests + 15 React jetsonSlice tests + 12 React jetsonClient tests · CLAUDE.md `§16 Glossary` extended with "Classroom Jetson v2.3.0 follow-up" entry). Earlier: 2026-05-16 (Classroom Jetson Orin Nano bundle v2.3.0: new Supabase migration `019_classroom_jetsons.sql` adds the `jetsons` table + 7 RPCs (`claim_jetson`, `release_jetson`, `heartbeat_jetson`, `agent_heartbeat_jetson`, `pair_jetson`, `force_release_jetson`, `sweep_jetson_locks`) with two new Postgres error codes `P0030 Jetson belegt → 409` and `P0031 Lock verloren → 410` · new Cloud API module `cloud_training_api/app/routes/jetson.py` exposes 8 endpoints (`POST /jetson/register` + `POST /jetson/{id}/agent-heartbeat` for the agent without JWT; `GET /classrooms/{id}/jetson` + `POST /jetson/{id}/claim` + `/heartbeat` + `/release` for the student; `POST /teacher/classrooms/{id}/jetson/pair` + `/force-release` for the teacher) and a new `app/services/jetson_sweep.py` 60 s loop that calls `sweep_jetson_locks()` to auto-release any lock whose `current_owner_heartbeat_at` is older than 5 min · new `robotis_ai_setup/jetson_agent/` directory ships the agent itself (long-running `agent.py` state machine, JWT-gated WebSocket bridge `rosbridge_proxy.py` on :9091, one-shot `setup.sh` installer, `docker-compose.jetson.yml` two-service stack, systemd unit, udev rules) — the agent runs PID 1 of the `edubotics-jetson.service`, sends a 10 s heartbeat to the Cloud API to learn the current owner, brings up the ROS containers on claim, runs a one-shot quintic move-to-safe-home `[0, -π/4, π/4, 0, 0, gripper=0.8]` after the stack is healthy, kills the proxy + tears down containers + `docker volume rm` of `jetson_workspace` + `jetson_huggingface_cache` on disconnect so every new student gets a pristine Jetson · `entrypoint_omx.sh` gains `EDUBOTICS_FOLLOWER_ONLY=1` short-circuit that skips the leader port wait + leader launch + quintic-sync + post-sync verification (leader stays at student desk; only the follower physically moves to the classroom Jetson) · `robotis_ai_setup/docker/build-images.sh` extended with `PLATFORM=arm64` flag — Phase 0 smoke test confirmed `robotis/ros:jazzy-ros-base-torch2.7.0-cuda12.8.0` IS multi-arch (saved 2-3 days vs from-sources base build); `robotis/open-manipulator:amd64-4.1.4` is amd64-only so we build the arm64 base ourselves once from the upstream `open_manipulator/docker/Dockerfile` (which uses multi-arch `ros:jazzy-ros-base`) — pushed as `nettername/open-manipulator-base:arm64-4.1.4`; both thin overlays now `ARG BASE_IMAGE` so one Dockerfile builds both architectures · React side: new `src/store/jetsonSlice.js` Redux slice, `src/services/jetsonClient.js` API wrapper, `src/hooks/useJetsonConnection.js` state machine (discovery on Inference-tab mount, 30 s heartbeat, navigator.sendBeacon release on tab unload), `src/components/JetsonAvailabilityChip.js` German UX chip ("Jetson frei" / "Jetson belegt von X" / "Verbunden mit Klassen-Jetson" / "Verbindung wird beendet…" on disconnect / "Verbindung wird wiederhergestellt…" on transient heartbeat failure), `rosConnectionManager.setAuthToken()` plumbs the Supabase JWT into the first WS frame as `{op: "auth", token: "<JWT>"}`, `StudentApp.js` sidebar hides Aufnahme + Roboter Studio when `state.jetson.status === 'connected'` (those need the local rosbridge which the Jetson session overrides) · 33 new cross-platform unit tests (20 cloud API: pairing code, mDNS name, online window, full P-code mapping, serialise; 13 jetson agent: docker hub reachability, manifest digest parsing arm64-preference, compose image discovery, last-pull persistence) · CI extended: `python-tests` job picks up both new test directories, `compose-validate` now also validates `docker-compose.jetson.yml`, `shell-lint` extended with `setup.sh` · scope is intentionally narrow — **ONLY the Inferenz tab connects to the Jetson**; Roboter Studio, Calibration, and Recording all stay on the student PC · VERSION + .iss + GUI fallback constant all bumped 2.2.4 → 2.3.0 so existing student installs prompt to re-install via the `/version`-poll update gate, picking up the new React build with the Jetson UI · new runbook `docs/JETSON_DEPLOY.md` for teachers + `docs/arm64_base/README.md` for the maintainer one-time arm64 base build · TLS in v1 is plain `ws://` + JWT only (documented as a v2 follow-up). Earlier: 2026-05-15 (ACT-quality + camera-sync bundle F62-F66 + auto-pull GUI 2.2.4 bundle F67-F69: **F67** offline short-circuit in `docker_manager.is_dockerhub_reachable()` — 5 s TCP probe to `registry-1.docker.io:443` skips the per-image retry storm when classrooms are offline · **F68** manifest-digest pre-check via `_get_remote_manifest_digest()` + `_get_local_repo_digest()` — picks the linux/amd64 entry from `docker manifest inspect`, compares to local RepoDigest, skips the real pull when they match (steady-state path drops from ~30 s of full pulls to ~3 s of HEAD requests) · **F69** last-pull persistence to `%LOCALAPPDATA%/EduBotics/.last_image_pull.json` with a "Letzter Update vor X Tagen" log line on next start + red banner past `IMAGE_FRESHNESS_WARN_DAYS=14` so teachers can spot classroom PCs that have been offline too long · VERSION + .iss + GUI fallback constant all bumped 2.2.3 → 2.2.4 so the existing `/version`-poll update gate (CLAUDE.md §6.2 step 1) prompts students with stale `.exe` installs to re-install on next launch, which is how the new auto-pull code itself reaches them · 20 new cross-platform unit tests in `tests/test_docker_auto_pull.py` covering offline detection, digest parsing, persistence roundtrip, and orchestration short-circuits. Earlier: F62-F66 deploy round (second pass also fixed F62's missing overlay-chain entry — `lerobot_dataset_wrapper.py` is now overlaid via `apply_overlay lerobot_dataset_wrapper.py "*/data_processing/*"` so the file actually reaches student installs, AND the deploy used `docker buildx build --push` to bypass Docker Desktop's dual-image-store gotcha that silently uploaded a stale `physical-ai-server:latest` on the first try: F62 mp4 preset `ultrafast→fast`/CRF 28→23 in `lerobot_dataset_wrapper.py:331` · F63 `--dataset.image_transforms.enable=true` always-on at train time in `training_handler._build_training_command` · F64 ACT-only default `--policy.n_action_steps=15` in same builder, overridable via `training_params['n_action_steps']` · F65 `ApproximateTimeSynchronizer`-style paired-camera capture in `overlays/communicator.py` via `_pick_synced_camera_msgs` with `_CAMERA_SYNC_SLOP_NS=15_000_000` slop and 8-entry per-camera ring · **F66 follow-up hardening landed same session from 4-agent verifier review**: (a) the F65 deque snapshot is now wrapped in a 3-attempt retry that catches `RuntimeError: deque mutated during iteration` (real risk under `MultiThreadedExecutor(num_threads=3)` in `physical_ai_server.py:2961`) and falls back to `camera_topic_msgs`; (b) `clear_latest_data()` now `.clear()`s the F65 sync rings so a re-recorded episode's first tick can't pair a fresh msg with a stale msg from the prior episode; (c) the F64 override path is now gated on `model_type == "act"` (closes the cross-policy leak where a `diffusion`/`pi0`/`vqbet` job carrying `n_action_steps` in `training_params` would emit `--policy.n_action_steps=N` for a policy whose config schema doesn't define that field); (d) the override is validated as `isinstance(int) and > 0` — `None`, `0`, negative, and non-int values fall back to the F64 default rather than emit a broken CLI arg or crash `ACTConfig.__post_init__`. The test suite (`tests/test_training_handler_cli.py`) was extended to 7 tests covering all 7 `ALLOWED_POLICIES`, the cross-policy leak, the invalid-override fallback, and the "exactly-once" emission count. Earlier: 2026-05-14 (Tier-1–9 audit-driven fix bundle: hardware safety [E1-E4, S1-S2, H1] · cloud API hardening [A1-A3, R3, N1 via migration 018, H4] · sensor + cloud-vision wiring [O1, O3, U6, N3] + M6 · calibration + tutorial UX [U1 via new `useSupabaseTutorialProgress` hook, U2, U3 via `requestRecalibration` action, U4] · real Modal vision smoke test [O2, N4] + N2 breakpoint-sync toast · German transliteration sweep + CI `german-strings-lint` job · CI hardening [H27 overlay-guard success case, H28 routes/training+vision SDK probe, H29 .s6-keep assertion] · installer + build hardening [H17, H21, H22, H23, H25, M14]). Earlier baseline: 2026-05-11 camera-pipeline F1-F61 bundle from commit `1b68372`.
...
├── VERSION                                ← "2.3.0" (consumed by gui/app/constants.py)
...
| 10 | Classroom Jetson Orin Nano — shared remote inference target (Inferenz tab only) | `robotis_ai_setup/jetson_agent/` (agent.py + rosbridge_proxy.py + setup.sh + docker-compose.jetson.yml + systemd unit + udev), `cloud_training_api/app/routes/jetson.py`, `cloud_training_api/app/services/jetson_sweep.py`, supabase migrations `019_classroom_jetsons.sql` + `020_jetson_v2.sql`, React `src/store/jetsonSlice.js` + `src/services/jetsonClient.js` + `src/hooks/useJetsonConnection.js` + `src/components/teacher/JetsonSection.js` + `src/components/teacher/PairJetsonModal.js` | Our code |
...
|---|---|---|---|
| Docker Hub | `nettername/*` (amd64 student) | 3 student images: `physical-ai-manager`, `physical-ai-server`, `open-manipulator` (all `:latest`, linux/amd64) | Free (public) |
| Docker Hub | `nettername/*-jetson*` (arm64 Jetson) | 4 Jetson images: `open-manipulator-jetson:latest`, `physical-ai-server-jetson:latest`, `open-manipulator-jetson-base:4.1.4`, `physical-ai-server-jetson-base:0.8.2` (all linux/arm64). Dedicated repos so a Jetson rollout never collides with the amd64 student stack. | Free (public) |
...
| Supabase | Postgres + Auth + Realtime | 11 user-facing tables (`users`, `trainings`, `classrooms`, `progress_entries`, `workflows`, `workgroups`, `workgroup_memberships`, `datasets`, `workflow_versions`, `tutorial_progress`, `jetsons`) + ~20 RPCs + Realtime publications for `trainings`, `workflows`, `workgroups`, `datasets`, `tutorial_progress`, `workflow_versions`, `jetsons` | Free tier |
...
Docker Engine runs inside a bundled WSL2 distro called `EduBotics`. The GUI invokes Docker via `wsl -d EduBotics -- docker ...` (wrapped by `_docker_cmd()` in `gui/app/docker_manager.py`). USB devices reach the distro via `usbipd attach --wsl EduBotics --busid X` (usbipd 5.x takes the distro as the positional value of `--wsl`; the older `--wsl --distribution <name>` form is rejected with "Unrecognized command or argument"). Reasons: no Docker Desktop license prompt, no tray sprawl, control over Docker version (pinned `5:27.5.1-1~ubuntu.22.04~jammy` + containerd `1.7.27-1`, both held via `apt-mark hold`), headless dockerd starts on distro boot via `/usr/local/bin/start-dockerd.sh` (re-spawn watchdog every 5s).
...
### 6.1 Install
`EduBotics_Setup.exe` (Inno Setup, AppId `{B7E3F2A1-8C4D-4E5F-9A6B-1D2E3F4A5B6C}`, AppVersion **2.3.0** in `installer/robotis_ai_setup.iss`, `PrivilegesRequired=admin`). Ships:
...
**Install diagnostics log** at `%LOCALAPPDATA%\EduBotics\install_diagnostics.log` (created by `install_prerequisites.ps1`, appended by `configure_usbipd.ps1` + `verify_system.ps1`, and by the GUI itself via `device_manager._append_diag()` when "Arme scannen" fails). Every section block is timestamped with an ISO date + a `script::section` tag. Captures: usbipd version + raw `usbipd list` + `usbipd policy list` before/after, `Get-PnpDevice` enumeration of `VID_2F5D` (Windows-side ground truth of whether the arms are visible), the `usbipd attach` smoke-test trace, and at scan time the parsed device list and the host→WSL→docker chain diagnosis result. This is the one file a teacher should ask for when a student reports an unexplained "Nicht gefunden".
...
2. `install_prerequisites.ps1` — Win11 ≥ build 22000, reject Home edition, virtualization warning, CFA warning, `wsl --install --no-distribution`, then **`wsl --update`** (idempotent — forces the Linux kernel current before any later `wsl --import` runs against it), download + install `usbipd-win` 5.3.0 MSI with **SHA256 verify** `1C984914AEC944DE19B64EFF232421439629699F8138E3DDC29301175BC6D938`. **Refreshes `$env:Path` from the system + user registry hives immediately after the MSI** so the remaining `[Run]` steps in the same Inno session can see `usbipd.exe` without a reboot (without this, `configure_usbipd.ps1` would silently skip on a fresh install because Inno's child shell inherited the pre-MSI PATH). **Probes `Get-PnpDevice -PresentOnly` for `VID_2F5D` devices** and logs the result to `install_diagnostics.log` — when a student later hits a hardware problem, this line tells support whether Windows ever saw the device at install time. Also probes `usbipd --version` from THIS shell as a PATH-refresh sanity check. On WSL kernel install drops `.reboot_required` marker.
...
4. `configure_usbipd.ps1` — adds `usbipd policy add` for ROBOTIS VID `2F5D` (PIDs `0103` + `2202` + any extra PID discovered in `usbipd list` at install time). Handles usbipd 4.x vs 5.x API drift via a **three-attempt cascade per PID**: `--operation AutoBind` → `--operation Connected` → bare `policy add`. Landing the policy is what matters; the syntactic flavor doesn't. **If a VID 2F5D device is plugged in at install time AND the EduBotics distro is already registered** (i.e. no pending reboot), runs a **real `usbipd attach --wsl EduBotics --busid <id>` smoke test** end-to-end then detaches — load-bearing proof on THIS machine that driver + usbipd + WSL kernel + policy are wired correctly before the student ever opens the GUI. Smoke-test failure does NOT break the install; the GUI's `_scan_arms` will surface a precise German diagnostic later (see §6.2 step 4).
...
7. `verify_system.ps1` — extended check: WSL2, distro, dockerd, **usbipd reachable + policy contains a `2F5D` rule** (warn if missing), **Windows currently sees `VID_2F5D` via `Get-PnpDevice` snapshot** (warns when arms are unplugged at verify time — informational), images, NVIDIA, install dir files. Exits non-zero on any hard FAIL (H23 — no more silent half-broken installs). Echoes the diagnostics log path so the teacher can find it without grepping `%LOCALAPPDATA%`.
...
3a. **Scan diagnostics path** — when the scan returns (None, None) the GUI calls `device_manager.diagnose_usb_environment(image=IMAGE_OPEN_MANIPULATOR)` which walks the same host→WSL→docker chain and returns a `UsbDiagnosis` dataclass whose single populated boolean tells the GUI which link broke: `usbipd_missing` / `wsl_distro_missing` / `windows_sees_no_robotis` (the most common — Windows itself doesn't enumerate VID 2F5D, almost always cable/power/wrong-port) / `usbipd_sees_no_robotis` / `attach_failed` / `no_serial_after_attach` / `docker_image_missing` / `ok` (chain works but `identify_arm.py` couldn't classify — usually wrong servo IDs). The GUI prints the populated `message_de` (a multi-line bulleted German error) into the log pane and the first line into the status bar, then echoes the path to `install_diagnostics.log` so a teacher can read the captured raw output. `list_usb_devices` itself logs every raw `usbipd list` invocation to that same file via `_append_diag()` regardless of success — so even "I scanned and got nothing" leaves forensic evidence. The list parser additionally tolerates usbipd 5.x "Connected:"/"Persisted:" section headers (Persisted entries are skipped because they have no BUSID) and falls back to a relaxed VID:PID-only regex when the strict state-column regex fails, so a future usbipd patch release with a new state string (e.g. "Forced") still yields a parsed device list.
...
**Authoring** — React `Workshop/` editor (Blockly 12.5.0 + react-blockly 9.0.0). The 2026-05 stripdown removed the static `ALLOWED_BLOCK_TYPES` frozensets from both `overlays/workflow/interpreter.py` and `cloud_training_api/app/validators/workflow.py` — handler dispatch is now the gate. The effective runtime allowlist is `STATEMENT_HANDLERS` + `VALUE_EVALUATORS` keys in `overlays/workflow/handlers/__init__.py`, `HAT_BLOCK_TYPES` in `overlays/workflow/interpreter.py`, plus the Blockly built-ins the interpreter dispatches on (controls_*, lists_*, logic_*, math_*, procedures_*, variables_*, text). Unknown block types raise `KeyError` out of the handler tables. The set the 2026-05 upgrade widened to:
...
| `STALLED_WORKER_MINUTES` | code default **`25`** (live Railway value is currently `15` — tighter than docs by operator choice; either value is valid, don't "correct" without checking which is intended) | `_sync_modal_status` cancels Modal job + marks failed if `last_progress_at` older |
...
| **P0030** | Jetson is already claimed by another student (claim race winner takes all) | 409 |
| **P0031** | Lock lost — caller is not the current owner of the Jetson (sweeper / teacher-force release / stale tab) | 410 Gone |
...
## 9. Supabase schema (base + 16 active numbered migrations through 020)

> **File map:** `migration.sql` (base) + `002_accounts.sql` + `003_lessons_and_notes.sql` (immediately superseded by 004) + `004_progress_entries.sql` + `005_cloud_job_id.sql` + `006_loss_history.sql` + `007_deletion_requested_at.sql` + `008_workflows.sql` + `009_workflows_rls_writes.sql` + `010_progress_terminal_guard.sql` + `011_workgroups.sql` + `012_dataset_sweep.sql` + `013_revoke_anon_from_security_definer.sql` + `015_workflow_versions.sql` + `016_tutorial_progress.sql` + `017_vision_quota.sql` + `018_workflow_versions_author_and_group_rls.sql` + `019_classroom_jetsons.sql` + `020_jetson_v2.sql`. **There is no `014_*.sql`** — the Phase-2 bundle skipped 014 to avoid a filename collision discovered mid-rollout (see note before §9.15). Every numbered migration has a matching file in `supabase/rollback/`; `migration.sql`, the superseded `003_*.sql`, and `018_*.sql` do not (018 is idempotent: `CREATE OR REPLACE` + `DROP POLICY IF EXISTS` + `CREATE POLICY`). Migration 020 is documented in §9.19a below.
...
### 9.19 019_classroom_jetsons.sql (Classroom Jetson Orin Nano support — inference target)
- New table `public.jetsons` (id, classroom_id FK ON DELETE SET NULL, agent_token UUID NOT NULL DEFAULT gen_random_uuid(), pairing_code TEXT UNIQUE NULL, pairing_code_expires_at TIMESTAMPTZ, mdns_name TEXT, lan_ip TEXT, agent_version TEXT, last_seen_at TIMESTAMPTZ, current_owner_user_id UUID FK ON DELETE SET NULL, current_owner_heartbeat_at TIMESTAMPTZ, claimed_at TIMESTAMPTZ, created_at, updated_at).
- Partial indexes: `idx_jetsons_current_owner WHERE current_owner_user_id IS NOT NULL`, `idx_jetsons_pairing_code WHERE pairing_code IS NOT NULL`, `idx_jetsons_owner_heartbeat WHERE current_owner_user_id IS NOT NULL` (sweeper).
- 7 SECURITY DEFINER RPCs (all `SET search_path = public`, `REVOKE FROM PUBLIC/anon/authenticated`, `GRANT TO service_role`):
  - `claim_jetson(p_jetson_id, p_user_id) RETURNS public.jetsons` — `SELECT ... FOR UPDATE`. Raises `P0030 Jetson belegt` if already held; `P0002` if not paired or not found.
  - `release_jetson(p_jetson_id, p_user_id) RETURNS VOID` — idempotent; only clears if caller IS the current owner.
  - `heartbeat_jetson(p_jetson_id, p_user_id) RETURNS VOID` — raises `P0031 Lock verloren` on owner mismatch so React can auto-disconnect.
  - `agent_heartbeat_jetson(p_jetson_id, p_agent_token, p_lan_ip, p_agent_version) RETURNS UUID` — agent's own 10 s heartbeat. Returns current_owner_user_id so the agent's rosbridge proxy knows which JWT sub to allow.
  - `pair_jetson(p_classroom_id, p_pairing_code, p_teacher_id, p_mdns_name) RETURNS UUID` — teacher binds device. Asserts classroom ownership; raises `P0011` if not the classroom's teacher.
  - `force_release_jetson(p_jetson_id, p_teacher_id) RETURNS VOID` — teacher emergency unlock.
  - `sweep_jetson_locks() RETURNS INTEGER` — called by `app/services/jetson_sweep.py` every 60 s; auto-releases any lock whose `current_owner_heartbeat_at < NOW() - INTERVAL '5 minutes'`.
- RLS: classroom members `SELECT` only their classroom's Jetson; teachers `SELECT/UPDATE` Jetsons in classrooms they own; admins all. Service-role bypasses (cloud API path).
- Realtime publication: `ALTER PUBLICATION supabase_realtime ADD TABLE public.jetsons` (DO-block-guarded, matches 011/015 pattern).
- BEFORE-UPDATE `trg_jetsons_touch` trigger keeps `updated_at` fresh.
- Two new Postgres error codes added to §7.5: `P0030 → 409 Jetson belegt`, `P0031 → 410 Lock verloren`.
- Rollback at `supabase/rollback/019_classroom_jetsons_rollback.sql`.

### 9.19a 020_jetson_v2.sql (Classroom Jetson v2.3.0 follow-up — lifecycle hardening)
- Three additional SECURITY DEFINER RPCs in the same pattern as 019 (`SET search_path = public`, REVOKE from PUBLIC/anon/authenticated, GRANT to service_role only):
  - `agent_release_jetson(p_jetson_id, p_agent_token) RETURNS VOID` — the **agent**'s own emergency release. Called when a local claim transition fails (docker-compose timeout, healthcheck flap, OOM) so the server-side lock frees within seconds instead of waiting the full 5 min for `sweep_jetson_locks`. Verifies the long-lived `agent_token` (NOT the per-session JWT). Closes the gap where a docker hiccup mid-claim left a student staring at "Jetson belegt von <ihrem eigenen Namen>" for 5 minutes.
  - `regenerate_pairing_code(p_jetson_id, p_teacher_id, p_new_code, p_expires_at) RETURNS VOID` — teacher rotates the 6-digit pairing code from the classroom-detail dashboard (no SSH back to the Jetson required). Cryptographic randomness stays in Python (`secrets.randbelow`); this RPC just atomically swaps the row's code + expiry. Cloud API wrapper retries once on the rare UNIQUE collision (~1-in-a-million).
  - `unpair_jetson(p_jetson_id, p_teacher_id) RETURNS VOID` — teacher unbinds device from classroom (sets `classroom_id=NULL`) and force-releases any active owner in one atomic transaction. `agent_token` is preserved so the same physical device can be re-paired (e.g., moved to a different classroom) without re-running `setup.sh`.
- All three raise `P0011` on teacher-classroom mismatch (matches 019's pattern) and `P0001` on agent-token mismatch.
- Rollback at `supabase/rollback/020_jetson_v2_rollback.sql` drops the 3 RPCs (the table + 019 RPCs are untouched).
- Boot-time schema fingerprint in `cloud_training_api/app/main.py:_validate_required_schema()` extended to probe all 3 new RPCs by name — a Railway deploy aborts at boot if 020 hasn't been applied yet.

### 9.20 Bootstrap admin (run once)
```bash
cd robotis_ai_setup
...
| `"Arme scannen" findet sofort nichts` | Read `%LOCALAPPDATA%\EduBotics\install_diagnostics.log` (created by installer, appended by GUI). The GUI's `_scan_arms` failure path also calls `device_manager.diagnose_usb_environment()` which prints a specific German error into the log pane: `usbipd_missing` (PATH issue — reboot), `windows_sees_no_robotis` (cable/power/USB-port — most common), `attach_failed` (policy never landed — re-run `configure_usbipd.ps1` elevated), `no_serial_after_attach` (driver/cable). |
...
Five sources of truth (verified in sync at 2026-05-16):
- `Testre/VERSION` → **`2.3.0`** (read by `gui/app/constants.py:APP_VERSION`)
- `installer/robotis_ai_setup.iss AppVersion` → **`2.3.0`**
...
**Rule**: When bumping product version, hit VERSION, .iss, and the GUI fallback constant (`gui/app/constants.py`) in the same change — AND bump the Railway `GUI_VERSION` + `GUI_DOWNLOAD_URL` env vars on the `scintillating-empathy` Cloud API service AFTER the matching GitHub release (`v<version>` tag with the `EduBotics_Setup.exe` asset) is published. Without the Railway+GitHub side, the in-tree bump is meaningless to students: `/version` keeps serving the old version, no upgrade prompt fires, and the new code never reaches existing installs. The 2.2.2 → 2.2.3 drift was closed by commit `e7f3fcb`. The 2.2.3 → 2.2.4 bump on 2026-05-15 shipped the auto-pull-on-start hardening — see §6.2 for the new freshness banner and the `is_dockerhub_reachable` / `_get_remote_manifest_digest` / `_save_last_pull_info` helpers in `docker_manager.py`. The 2.2.4 → 2.3.0 bump on 2026-05-16 shipped the **Classroom Jetson Orin Nano** feature (migration 019 + 020 + Cloud API `/jetson/*` routes + `robotis_ai_setup/jetson_agent/` + arm64 image set on dedicated `nettername/{open-manipulator,physical-ai-server}-jetson{,-base}` repos + React Jetson UX on the Inference tab — see §3 layer 10, §9.19 / §9.19a, `docs/JETSON_DEPLOY.md`, `docs/arm64_base/README.md`). Bumping `VERSION` is what arms the existing `/version`-poll update gate, prompting students with stale `.exe` installs to re-install when their GUI next launches; without the bump, the new code never reaches their machine.

---

## 15. CI workflow (`.github/workflows/ci.yml`) — what fails the build

11 jobs run on every push/PR to `main` (in the order they appear in `ci.yml`):
...
8. **tutorials-validate** — JSON-parses every `physical_ai_manager/public/tutorials/*.json`, asserts the required schema (`id`, `title_de`, `level`, `steps[].title`, `steps[].body`, `steps[].allowed_blocks`), and cross-checks each `allowed_blocks` entry against the runtime dispatch tables: `STATEMENT_HANDLERS` + `VALUE_EVALUATORS` keys from `overlays/workflow/handlers/__init__.py`, `HAT_BLOCK_TYPES` from `overlays/workflow/interpreter.py`, plus a static list of Blockly built-ins the interpreter dispatches (kept in sync with the interpreter's btype ladder). A tutorial referencing a block that doesn't exist server-side fails the build. The previous static `ALLOWED_BLOCK_TYPES` source-of-truth was removed in the 2026-05 stripdown.
...
- **Classroom Jetson** — shared NVIDIA Jetson Orin Nano 8 GB Dev Kit that runs the ROS inference stack on `linux/arm64`. Scope is intentionally narrow: **ONLY the Inferenz tab connects to it**; Roboter Studio, Calibration, and Recording all stay on the student PC. One Jetson per classroom (paired via 6-digit code via `pair_jetson` RPC), first-come explicit-disconnect lock model (no queue), wipe-on-release lifecycle (every new student gets a pristine `huggingface_cache` + `workspace` volume so previous downloads don't leak). v2.3.0 (2026-05-16). Source: `robotis_ai_setup/jetson_agent/` (`setup.sh`, `agent.py`, `rosbridge_proxy.py`, `docker-compose.jetson.yml`, systemd unit, udev rules). **v2.3.0 follow-up (2026-05-17, this commit set)**: separate Docker Hub repos for the Jetson images per Sven's "dont touch our EduBotics images" rule — now runs `nettername/open-manipulator-jetson:latest` + `nettername/physical-ai-server-jetson:latest` (built from `nettername/open-manipulator-jetson-base:4.1.4` + `nettername/physical-ai-server-jetson-base:0.8.2`). NO physical_ai_manager (React stays on the student PC). See `docs/JETSON_DEPLOY.md` for the teacher runbook and `docs/arm64_base/README.md` for the one-time arm64 base build.
- **Classroom Jetson v2.3.0 follow-up (2026-05-17)** — closed the four classroom-blocking gaps from the deep audit: (1) teacher dashboard `JetsonSection` + `PairJetsonModal` + Cloud API teacher endpoints (`/regenerate-code`, `/unpair`) so teachers can pair/manage/unpair without SSH; (2) `navigator.sendBeacon` release now hits a dedicated `/jetson/{id}/release-beacon` endpoint that accepts a Bearer-in-body JWT (the previous query-param hack silently failed auth and orphaned every tab-close for 5 min); (3) `resetJetsonOnLogout` wired into all 5 signOut callsites so logout/session-timeout releases the lock instead of waiting for the sweeper; (4) missing `/jetson/{id}/agent-release` endpoint added so the agent can free its server-side lock on local claim failures (docker-compose hiccup, OOM) instead of relying on the 5-min sweeper. New Supabase migration `020_jetson_v2.sql` with 3 RPCs (`agent_release_jetson`, `regenerate_pairing_code`, `unpair_jetson`). Cloud API also forwards the Supabase JWT secret to the agent at `/jetson/register` time when `SUPABASE_JWT_ALGORITHM=HS256` (required for legacy-auth Supabase projects — without it the rosbridge proxy would reject every student JWT with close code 4401). Heartbeat now flips a `heartbeatTransient` Redux flag after 2 consecutive non-410 failures so the chip shows "Verbindung wird wiederhergestellt…" amber variant during Wi-Fi blips. New Jest tests for `jetsonSlice` (15 cases) + `jetsonClient` (12 cases) lock the sendBeacon fix against regression.
- **Jetson agent** — `robotis_ai_setup/jetson_agent/agent.py`, PID 1 of `edubotics-jetson.service`. State machine that heartbeats to `POST /jetson/{id}/agent-heartbeat` every 10 s, observes `current_owner_user_id` transitions, brings up containers on claim, moves follower to safe home `[0, -π/4, π/4, 0, 0, gripper=0.8]` via `docker exec` + a one-shot quintic publisher, starts `rosbridge_proxy.py`, and on release tears everything down + `docker volume rm` the per-session volumes. Ports the F67/F68/F69 auto-pull pattern (TCP probe to `registry-1.docker.io:443`, `docker manifest inspect` for arm64 digest, last-pull persistence at `/var/lib/edubotics/.last_image_pull.json`).
- **Jetson rosbridge proxy** — `robotis_ai_setup/jetson_agent/rosbridge_proxy.py`, Python `websockets` server on `0.0.0.0:9091`. The Jetson's rosbridge is bound to `127.0.0.1:9090` (loopback only — unauthenticated `:9090` NEVER reaches the LAN). On WS accept, the proxy expects the first frame to be `{"op": "auth", "token": "<Supabase JWT>"}`, verifies via JWKS (RS256) or shared secret (HS256), checks `sub == current_owner_user_id` (read from the agent's loopback `/owner` endpoint cached for 1 s), and then transparently pipes bytes to `127.0.0.1:9090`. Close codes: 4400 no-auth-frame, 4401 invalid JWT, 4403 wrong owner. No TLS in v1 — documented as a v2 follow-up.
- **agent_token** — long-lived per-Jetson UUID (`jetsons.agent_token`, default `gen_random_uuid()`). Minted at `/jetson/register` first-boot, written to `/etc/edubotics/jetson.env` mode 600 (root only). Sent in every agent heartbeat to `agent_heartbeat_jetson` RPC; mismatch raises P0001. Mirrors the `trainings.worker_token` shape — service-role + token-verify is the only thing that lets the agent learn the current owner.
- **Follower-only mode** — `EDUBOTICS_FOLLOWER_ONLY=1` env gate in `entrypoint_omx.sh` (added 2026-05-16). Skips the leader port wait, the `omx_l_leader_ai.launch.py` launch, the leader-pose read, and the quintic-sync trajectory + post-sync verification. Used on the classroom Jetson because the leader arm physically stays at the student's desk. The Jetson agent moves the follower to safe home via its own one-shot publisher after the container is healthy.
- **P0030 / P0031** — see §7.5. P0030 (409 Jetson belegt) raised by `claim_jetson` when `current_owner_user_id IS NOT NULL`; P0031 (410 Lock verloren) raised by `heartbeat_jetson` when the caller is no longer the current owner. React maps 410 to auto-disconnect (toast "Verbindung zum Jetson verloren — bitte erneut verbinden") + flips the chip back to "Jetson frei".
