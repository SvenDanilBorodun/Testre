...
- **On-demand follower sync + verification** in `docker/open_manipulator/arm_startup_node.py` (v2.4.2+). The follower no longer syncs at boot — it stays LIMP (xacro `disable_torque_at_init: true`) until the student clicks "Roboter starten", which calls the `/edubotics/start_arm` (`std_srvs/Trigger`) service. The node homes the follower, reads the leader's current pose, runs the quintic sync (relocated verbatim from the old entrypoint Phase-3 heredoc), then **verifies convergence as a BLOCKING gate** — 0.30 rad per-arm-joint tolerance (gripper exempt) + ≥50% commanded-motion check. Only a passing verify publishes phase `ready` on the latched `/edubotics/arm_state` topic, which is the single gate the React client uses to unlock Aufnahme/Inferenz/Roboter-Studio. (Pre-v2.4.2 this was a soft-fail boot-time check in the entrypoint — upstream's tight 0.08 rad / hard-exit 2 had already been relaxed to 0.30 rad on 2026-05-18; the move to a node-side blocking gate restores a true fail-closed readiness check. See `docs/on-demand-arm-startup.md`.) `FOLLOWER_ONLY=1` (Jetson) skips the node — the Jetson agent homes the follower itself.
...
- **`EDUBOTICS_*` env vars only reach the container if listed in `docker-compose.yml::environment`.** Compose's `${VAR}` interpolation operates on YAML at config-time, NOT on the container's runtime env. An override in the host `.env` reaches compose's YAML substitution but is invisible to the entrypoint unless explicitly forwarded. The 2026-05 raw_mjpeg-override regression was exactly this: every student container ran with the entrypoint hardcoded default because the `EDUBOTICS_CAMERA_*` keys were never in the `environment:` list. Guarded by `ci.yml::env-forwarding-guard` since v2.3.7. Extended 2026-05-23 to also scan `robotis_ai_setup/docker/physical_ai_server/overlays/**/*.py` and `robotis_ai_setup/docker/open_manipulator/overlays/**/*.py` — overlay modules read `os.environ.get('EDUBOTICS_*')` at import time, so a missing compose-forward silently breaks operator overrides for `EDUBOTICS_MAX_BUFFER_GB`, `EDUBOTICS_CALIB_DIR`, `EDUBOTICS_DETECTOR`, `EDUBOTICS_YOLOX_ONNX`, `EDUBOTICS_DFINE_ONNX` etc. Extended again 2026-05-23 to scan `docker/open_manipulator/camera_ingest_node.py` (the native-bridge ingest node reads `EDUBOTICS_CAMERA_INGEST_PORT` / `EDUBOTICS_CAMERA_NAMES` at startup). Every `EDUBOTICS_*` referenced by `entrypoint_omx.sh`, `start-dockerd.sh`, `camera_ingest_node.py`, OR any overlay `.py` must appear in some compose file's `environment:` list, or CI fails.

- **Aufnahme recording RAM is capped at 4 GB by the safety valve, not by architecture.** The recording path decodes each `CompressedImage` to a 640×480 RGB uint8 ndarray (921 600 bytes/frame, ~0.92 MB) and appends to `lerobot_dataset_wrapper.episode_buffer` until `save_episode_*` runs end-of-episode. At 30 Hz × 2 cameras that's **1.84 MB/tick** → unbounded growth → without the valve the container OOMkills around frame index ~3 250 (6 GB cgroup divided by 1.84 MB/tick ≈ 109 ticks/s × 30 s — actually ~3 260 ticks total ≈ **~108 seconds** before the 6 GB cgroup cap is hit). The 2026-05 safety valve (`_episode_image_bytes` counter + `_buffer_full` flag in `lerobot_dataset_wrapper.py`, polled in `data_manager.py::run` state) forces a save when the buffer crosses `EDUBOTICS_MAX_BUFFER_GB` (default 4 GB). **Hard upper bound on episode length with the default 4 GB cap: ~74 seconds** (4 GiB / 1.84 MB-per-tick / 30 Hz). The previously documented "~110 s" was the OOMkill point at 6 GB, not the valve cap at 4 GB — corrected 2026-05-23. A `30 Sekunden` or `1 Minute` recording fits; the dashboard's "5 Minuten" option short-circuits via the safety valve at ~25 % of the requested duration with the German `[WARNUNG]`. The proper fix (store JPEG bytes during recording, decode chunked inside `FFmpegEncoder`) is deferred to v2.4 — ~50× headroom. If you raise `EDUBOTICS_MAX_BUFFER_GB`, also raise `mem_limit` in `docker-compose.yml` to leave headroom for the rest of the node.
