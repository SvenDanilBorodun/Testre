...
- **Installer upgrades DESTROY the student's Docker volumes (silent data loss).** `installer/scripts/import_edubotics_wsl.ps1` unconditionally runs `wsl --unregister EduBotics` before re-importing the rootfs on every upgrade install ("upgrade path" per its own comment), deleting the distro VHDX and with it `ai_workspace`, `huggingface_cache`, `edubotics_calib` — un-uploaded recordings, the local HF cache, and the 15-minute Roboter-Studio calibration. Confirmed live 2026-06-05 on the reference rig (2.5.x→2.6.0 upgrade: zero volumes afterwards). For 2.5.0→2.6.0 the wipe was semi-wanted (stale v2.1 datasets / pre-0.5.1 checkpoints), but as standing behavior it's wrong. Proposed fix (NOT yet implemented): record the imported rootfs sha256 (the `.sha256` sidecar already ships) and skip unregister+import when unchanged; when a re-import IS needed, surface a German confirmation naming what will be lost. Until then, German release notes must tell students to upload datasets before upgrading.
