...
4. `configure_usbipd.ps1` — adds `usbipd policy add` for ROBOTIS VID `2F5D` (PIDs `0103` + `2202` + any extra PID discovered in `usbipd list` at install time). Handles usbipd 4.x vs 5.x API drift via a **three-attempt cascade per PID**: `--operation AutoBind` → `--operation Connected` → bare `policy add`. Landing the policy is what matters; the syntactic flavor doesn't. **If a VID 2F5D device is plugged in at install time AND the EduBotics distro is already registered** (i.e. no pending reboot), runs a **real `usbipd attach --wsl --distribution EduBotics --busid <id>` smoke test** end-to-end then detaches — load-bearing proof on THIS machine that driver + usbipd + WSL kernel + policy are wired correctly before the student ever opens the GUI. Smoke-test failure does NOT break the install; the GUI's `_scan_arms` will surface a precise German diagnostic later (see §6.2 step 4).
