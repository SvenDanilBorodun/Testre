...
export default function ModelWeightSelector() {
  ...
  const handleModelWeightSelection = useCallback(
    (modelWeightPath) => {
      ...
      toast.success(`Modellgewicht ausgewählt: ${modelWeightPath}`);
    },
    [dispatch]
  );
  ...
  const classModelWeightItem = (isSelected) =>
    clsx(
      ...
      'border-gray-200',
      {
        'bg-teal-50 hover:bg-teal-100': isSelected,
  ...
  const classModelWeightName = (isSelected) =>
    clsx('text-sm', 'flex-1', {
      'text-teal-700 font-medium': isSelected,
      'text-gray-700': !isSelected,
    });

  const classSelectedIcon = clsx('w-5', 'h-5', 'mr-3', 'text-teal-600');

  return (
    <div className="flex flex-col items-center justify-center">
      <div className={classCard}>
        <div className={classTitle}>Modellgewichte</div>

        {selectedModelWeight && (
          <div className={classCurrentSelection}>
            <b>Ausgewählt:</b> {selectedModelWeight}
          </div>
        )}
        ...
        <div className={classListContainer}>
          {modelWeightList && modelWeightList.length > 0 ? (
            ...
            <div className="text-center text-gray-500 py-8">
              {loading ? 'Modellgewichte werden geladen...' : 'Keine Modellgewichte verfügbar'}
            </div>
          )}
        </div>
      </div>
