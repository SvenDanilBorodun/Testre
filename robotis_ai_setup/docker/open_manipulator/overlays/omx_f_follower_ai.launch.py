...
#
# EduBotics overlay (2026-05-18): wrap `robot_description` in
# `ParameterValue(..., value_type=str)`. Upstream passes the xacro Command
# output as a bare dict value, which launch_ros >= 0.22 type-infers as YAML.
# The follower xacro happens to contain content that fails the YAML parse
# (it works for the leader by accident), so the launch crashes with
# "Unable to parse the value of parameter robot_description as yaml".
# Without this fix the follower controller_manager never starts → no
# /joint_states topic → no /arm_controller subscription on /leader/joint_trajectory
# → teleoperation silently broadcasts into the void. (3 places: control_node,
# robot_controller_spawner, robot_state_publisher_node — line 145, 162, 169 below.)
# Also preserves the magic remap on line ~155 that makes /leader/joint_trajectory
# drive the follower's arm_controller (the architectural choice in CLAUDE.md §5.5).
...
def generate_launch_description():
    ...
    # EduBotics fix: tell launch_ros the xacro output is a string, not YAML.
