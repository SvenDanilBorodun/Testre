...
# EduBotics overlay (LeRobot v0.5.1 upgrade): LerobotTrainer is imported
# LAZILY inside _get_trainer(), NOT at module load. EduBotics trains in the
# cloud (Modal runs `lerobot-train` directly), so the local LerobotTrainer is
# dead weight here — but the upstream module-level import pulls in
# lerobot_trainer.py, which references several symbols that MOVED in v0.5.1
# (lerobot.scripts.eval → lerobot.scripts.lerobot_eval,
#  lerobot.utils.wandb_utils → lerobot.rl.wandb_utils,
#  lerobot.utils.utils.get_safe_torch_device → lerobot.utils.device_utils).
# Importing it at boot would crash the ROS node. Deferring the import keeps the
# node alive; if local training is ever invoked it fails at use-time with a
# clear ImportError instead of taking the whole server down at startup.
...
class TrainingManager:
    ...
    # Supported policy types that map to the (lazily imported) LerobotTrainer.
    # v0.5.1 renamed the pi0-fast registered type 'pi0fast' → 'pi0_fast' and
    # added 'pi05'. Kept as a set of names (not a name→class map) so the class
    # is not referenced at module load — see the import note above.
    ...
    def _get_trainer(self):
        ...
        if policy_type not in self.LEROBOT_POLICY_TYPES:
            raise ValueError(
                f'Supported types: {sorted(self.LEROBOT_POLICY_TYPES)}'
            )
        # Lazy import — see the module-level note. Only happens if local
        # training is actually invoked (not on the cloud-training student path).
        from physical_ai_server.training.trainers.lerobot.lerobot_trainer import (
            LerobotTrainer,
        )
