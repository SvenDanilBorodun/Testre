...
def _toKdlJoint(jnt):
    ...
    # NOTE (Roboter Studio vendor patch): upstream uses ``kdl.Joint.None``
    # which is a Python 2 attribute name and a SyntaxError in Python 3.
    # Modern PyKDL exposes the same JointType under ``Fixed``; ROS
    # distributors apply the same rename via Debian patch. We do it inline
    # so the file is importable directly.
