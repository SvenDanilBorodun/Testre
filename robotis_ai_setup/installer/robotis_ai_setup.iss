...
; The offline bundle's digest sidecar is INSTALL-TIME generated by
; load_images.ps1 (bundle mode only). Wipe it before every install so an
; online / self-update reinstall (which never re-creates it) cannot leave a
; stale OLD-tag sidecar behind; a bundle install re-seeds it in Step 5.
...
; The install-time-generated digest sidecar (not a tracked [Files] entry, so
; Inno's uninstall wouldn't otherwise remove it).
Type: files; Name: "{app}\docker\bundled_digests.json"
