...
function Wait-DockerReady {
    ...
    # a NativeCommandError record on every poll. GetTempFileName() returns a
    # normalized LONG path — NEVER build this under $env:TEMP, because a dotted
    # Windows username yields an 8.3 tilde path that crashes -LiteralPath binding
    # with a terminating PSArgumentException.
