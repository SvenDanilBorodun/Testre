-- Rollback for 028_training_log_url: restore the 7-arg RPC (migration 010
-- body, verbatim) and drop the column. Take a PITR snapshot first.
--
-- NOTE the same signature rule as the forward migration: the 8-arg
-- function must be DROPped (not replaced) before recreating the 7-arg
-- one, or both would coexist as overloads and PostgREST rpc() calls
-- would be ambiguous.

DROP FUNCTION IF EXISTS public.update_training_progress(
  INT, UUID, TEXT, INT, INT, REAL, TEXT, TEXT);
...
ALTER TABLE public.trainings DROP COLUMN IF EXISTS log_url;
