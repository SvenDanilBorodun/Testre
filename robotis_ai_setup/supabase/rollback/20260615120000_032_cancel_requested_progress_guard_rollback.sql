-- Rollback for 032: restore the migration-028 update_training_progress body
-- (terminal guard WITHOUT 'cancel_requested', original P0001 message).
--
-- Reverting re-opens the cancel cost-bomb described in 032 — only use to back
-- out 032 if it ever misbehaves. Signature is unchanged, so this is a plain
-- CREATE OR REPLACE; grants are preserved (re-granted for idempotency).
